...
def condition_trial_curves(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Trial-by-trial accuracy/RT curves for each condition and a convergence summary.

    Accuracy: mean correctness at each learning trial index within each condition.
    RT: mean correct-trial voice onset RT at each learning trial index within each condition.
    Convergence estimate per subject-condition:
      - accuracy_convergence = mean accuracy over the last up-to-27 learning trials
      - rt_convergence = mean correct-trial RT over the last up-to-10 correct learning trials
    Group summary averages these subject-level convergence estimates by condition.
    """
    learning = df[df["phase"] == "learning"].copy()
    if learning.empty:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()

    # Ensure trial index exists and is numeric
    if 'trial_in_condition' not in learning.columns:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    ...
    acc_curve = learning.groupby(['condition_name', 'trial_in_condition'], dropna=False).agg(
        accuracy_mean=('is_correct', 'mean'),
        accuracy_sem=('is_correct', sem_numeric),
        n_trials=('is_correct', 'size'),
        n_subjects=('subject_id', 'nunique'),
    ).reset_index()
    cat = pd.Categorical(acc_curve['condition_name'], categories=CONDITION_ORDER_NOMINAL, ordered=True)
    acc_curve = acc_curve.assign(_order=cat).sort_values(['_order', 'trial_in_condition']).drop(columns='_order')

    rt_correct = learning[(learning['is_correct'] == 1) & learning['voice_onset_rt_sec'].notna()].copy()
    rt_correct['voice_onset_rt_sec'] = pd.to_numeric(rt_correct['voice_onset_rt_sec'], errors='coerce')
    rt_correct = rt_correct[(rt_correct['voice_onset_rt_sec'] >= 0.05) & (rt_correct['voice_onset_rt_sec'] <= 8.0)]
    rt_curve = rt_correct.groupby(['condition_name', 'trial_in_condition'], dropna=False).agg(
        rt_mean=('voice_onset_rt_sec', 'mean'),
        rt_sem=('voice_onset_rt_sec', sem_numeric),
        n_correct_rt=('voice_onset_rt_sec', 'size'),
        n_subjects=('subject_id', 'nunique'),
    ).reset_index()
    cat = pd.Categorical(rt_curve['condition_name'], categories=CONDITION_ORDER_NOMINAL, ordered=True)
    rt_curve = rt_curve.assign(_order=cat).sort_values(['_order', 'trial_in_condition']).drop(columns='_order')

    # Subject-level convergence estimates
    rows = []
    for (subject_id, condition_name), sub in learning.groupby(['subject_id', 'condition_name'], dropna=False):
        ...
        acc_conv = float(pd.to_numeric(last_acc, errors='coerce').mean()) if len(last_acc) else float('nan')
        # RT convergence from last correct trials only
        subc = sub[(sub['is_correct'] == 1) & pd.to_numeric(sub['voice_onset_rt_sec'], errors='coerce').notna()].copy()
        subc['voice_onset_rt_sec'] = pd.to_numeric(subc['voice_onset_rt_sec'], errors='coerce')
        subc = subc[(subc['voice_onset_rt_sec'] >= 0.05) & (subc['voice_onset_rt_sec'] <= 8.0)]
        last_rt = subc['voice_onset_rt_sec'].tail(min(10, len(subc)))
        rt_conv = float(last_rt.mean()) if len(last_rt) else float('nan')
        rows.append({
    ...
    if conv_subj.empty:
        return acc_curve, rt_curve, pd.DataFrame()
    ...
    return acc_curve, rt_curve, conv_group



def plot_condition_trial_curves(acc_curve: pd.DataFrame, rt_curve: pd.DataFrame, conv_group: pd.DataFrame, out_dir: Path):
    """Save trial-wise accuracy and RT curves by condition, with convergence annotations."""
    if not acc_curve.empty:
        fig, ax = plt.subplots(figsize=(10.5, 5.8))
        for cname in CONDITION_ORDER_NOMINAL:
            sub = acc_curve[acc_curve['condition_name'] == cname].sort_values('trial_in_condition')
            if sub.empty:
                continue
            ax.plot(sub['trial_in_condition'], sub['accuracy_mean'], label=cname)
            # Convergence annotation near the end of the line
            if not conv_group.empty and cname in set(conv_group['condition_name']):
                ...
                if pd.notna(y):
                    ...
                    x_end = float(sub['trial_in_condition'].max())
                    ax.text(x_end + 0.5, y, f"≈{y:.2f}", fontsize=8, va='center')
        ax.set_title('Accuracy change by learning trial for each condition')
        ax.set_xlabel('Learning trial index within condition')
        ax.set_ylabel('Accuracy')
        ax.set_ylim(0, 1.05)
        ax.grid(alpha=0.3)
        ax.legend(fontsize=8, ncol=2)
        fig.tight_layout()
        fig.savefig(out_dir / 'accuracy_curve_by_trial_condition.png', dpi=220)
        plt.close(fig)

        # Small multiples, one subplot per condition
        ...
        for ax, cname in zip(axes, CONDITION_ORDER_NOMINAL):
            ...
            ax.set_title(cname)
            if not sub.empty:
                ax.plot(sub['trial_in_condition'], sub['accuracy_mean'])
                if 'accuracy_sem' in sub.columns:
                    low = sub['accuracy_mean'] - sub['accuracy_sem'].fillna(0)
                    high = sub['accuracy_mean'] + sub['accuracy_sem'].fillna(0)
                    ax.fill_between(sub['trial_in_condition'], low, high, alpha=0.2)
                if not conv_group.empty and cname in set(conv_group['condition_name']):
                    row = conv_group.loc[conv_group['condition_name'] == cname].iloc[0]
                    y = row.get('accuracy_convergence_mean', np.nan)
                    if pd.notna(y):
            ...
            ax.set_xlabel('Trial')
            ax.set_ylabel('Accuracy')
        fig.suptitle('Condition-wise accuracy curves (trial level)', y=1.02)
        fig.tight_layout()
        fig.savefig(out_dir / 'accuracy_curve_by_trial_condition_panels.png', dpi=220, bbox_inches='tight')
        plt.close(fig)

    if not rt_curve.empty:
        fig, ax = plt.subplots(figsize=(10.5, 5.8))
        for cname in CONDITION_ORDER_NOMINAL:
            sub = rt_curve[rt_curve['condition_name'] == cname].sort_values('trial_in_condition')
            if sub.empty:
                continue
            ax.plot(sub['trial_in_condition'], sub['rt_mean'], label=cname)
            if not conv_group.empty and cname in set(conv_group['condition_name']):
                row = conv_group.loc[conv_group['condition_name'] == cname].iloc[0]
                ...
                if pd.notna(y):
                    ax.axhline(y, linestyle='--', linewidth=0.8, alpha=0.25)
                    x_end = float(sub['trial_in_condition'].max())
                    ax.text(x_end + 0.5, y, f"≈{y:.2f}s", fontsize=8, va='center')
        ax.set_title('Voice onset RT change by learning trial for each condition')
        ax.set_xlabel('Learning trial index within condition')
        ax.set_ylabel('Voice onset RT (s)')
        ax.grid(alpha=0.3)
        ax.legend(fontsize=8, ncol=2)
        fig.tight_layout()
        fig.savefig(out_dir / 'rt_curve_by_trial_condition.png', dpi=220)
        plt.close(fig)
        ...
        for ax, cname in zip(axes, CONDITION_ORDER_NOMINAL):
            ...
            ax.set_title(cname)
            if not sub.empty:
                ax.plot(sub['trial_in_condition'], sub['rt_mean'])
                if 'rt_sem' in sub.columns:
                    low = sub['rt_mean'] - sub['rt_sem'].fillna(0)
                    high = sub['rt_mean'] + sub['rt_sem'].fillna(0)
                    ax.fill_between(sub['trial_in_condition'], low, high, alpha=0.2)
                if not conv_group.empty and cname in set(conv_group['condition_name']):
                    row = conv_group.loc[conv_group['condition_name'] == cname].iloc[0]
                    y = row.get('rt_convergence_mean', np.nan)
            ...
            ax.set_xlabel('Trial')
            ax.set_ylabel('RT (s)')
        fig.suptitle('Condition-wise RT curves (trial level)', y=1.02)
        fig.tight_layout()
        fig.savefig(out_dir / 'rt_curve_by_trial_condition_panels.png', dpi=220, bbox_inches='tight')
        plt.close(fig)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze tactile consonant learning experiment logs. v8: adds trial-wise condition accuracy/RT curves and convergence summaries.")
    ...
    cond_acc_curve, cond_rt_curve, condition_convergence = condition_trial_curves(df)
    ...
    cond_acc_curve.to_csv(out_dir / "condition_accuracy_curve_by_trial.csv", index=False, encoding="utf-8-sig")
    cond_rt_curve.to_csv(out_dir / "condition_rt_curve_by_trial.csv", index=False, encoding="utf-8-sig")
    condition_convergence.to_csv(out_dir / "condition_convergence_summary.csv", index=False, encoding="utf-8-sig")
    ...
    try:
        with pd.ExcelWriter(out_dir / "analysis_summary.xlsx", engine="openpyxl") as writer:
            ...
            cond_acc_curve.to_excel(writer, sheet_name="cond_acc_curve_trial", index=False)
            cond_rt_curve.to_excel(writer, sheet_name="cond_rt_curve_trial", index=False)
    ...
    plot_condition_trial_curves(cond_acc_curve, cond_rt_curve, condition_convergence, out_dir)
    plot_convergence_bars(condition_convergence, out_dir)
