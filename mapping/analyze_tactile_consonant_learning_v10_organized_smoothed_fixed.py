...
python analyze_tactile_consonant_learning_v7_rt_by_item.py --data-dir C:/path/to/logs --out-dir C:/path/to/analysis_results
"""
...
def make_output_dirs(out_dir: Path) -> Dict[str, Path]:
    """Create topic-specific output folders and return path aliases."""
    dirs = {
        "root": out_dir,
        "tables": out_dir / "01_tables",
        "condition": out_dir / "02_condition_summary",
        "curves": out_dir / "03_condition_curves",
        "item_learning": out_dir / "04_item_learning_speed",
        "rt": out_dir / "05_reaction_time",
        "confusion": out_dir / "06_confusion_matrices",
        "effects": out_dir / "07_symbol_position_effects",
        "report": out_dir / "08_report",
    }
...
def output_index_message() -> str:
    return """Output folders:
  01_tables                  cleaned data and Excel workbook
  02_condition_summary       condition-level bars and summary CSVs
  03_condition_curves        smoothed/binned accuracy and RT curves by condition
  04_item_learning_speed     consonant/position exposure-count learning speed
  05_reaction_time           RT analyses by consonant and position
  06_confusion_matrices      learning/retention confusion matrices
  07_symbol_position_effects symbol-vs-position exploratory summaries and heatmaps
  08_report                  markdown report
"""
...
def condition_trial_curves(
    df: pd.DataFrame,
    bin_size: int = 9,
    rolling_window: int = 9,
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Smoothed/binned accuracy and RT curves for each condition.

    Why this version:
    - Trial-level means can jump between 0 and 1 when the number of subjects is small.
    - This function first smooths within each subject-condition, then averages across subjects.
    - It also creates fixed-size trial bins, e.g., 1-9, 10-18, ..., to show meaningful group trends.

    Outputs
    -------
    subj_bin:
        subject-condition-bin accuracy and correct-trial RT
    group_bin:
        group mean/SEM of binned accuracy and RT by condition
    subj_roll:
        subject-condition-trial rolling accuracy and rolling RT
    group_roll:
        group mean/SEM of rolling curves by condition and trial
    conv_group:
        convergence summary by condition
    """
    learning = df[df["phase"] == "learning"].copy()
    if learning.empty or "trial_in_condition" not in learning.columns:
        empty = pd.DataFrame()
        return empty, empty, empty, empty, empty

    learning["trial_in_condition"] = pd.to_numeric(learning["trial_in_condition"], errors="coerce")
    learning = learning.dropna(subset=["trial_in_condition"])
    learning["trial_in_condition"] = learning["trial_in_condition"].astype(int)
    learning["is_correct"] = pd.to_numeric(learning["is_correct"], errors="coerce")
    learning["voice_onset_rt_sec"] = pd.to_numeric(learning["voice_onset_rt_sec"], errors="coerce")

    # ------------------------------------------------------------------
    # A) Binned curves: subject-level first, then condition-level average
    # ------------------------------------------------------------------
    learning["trial_bin_index"] = ((learning["trial_in_condition"] - 1) // int(bin_size)) + 1
    learning["trial_bin_start"] = (learning["trial_bin_index"] - 1) * int(bin_size) + 1
    learning["trial_bin_end"] = learning["trial_bin_index"] * int(bin_size)
    learning["trial_bin_label"] = learning["trial_bin_start"].astype(str) + "-" + learning["trial_bin_end"].astype(str)

    subj_bin = learning.groupby(
        ["subject_id", "condition_name", "trial_bin_index", "trial_bin_label"],
        dropna=False,
    ).agg(
        accuracy=("is_correct", "mean"),
        n_trials=("is_correct", "size"),
    ...
    group_bin = subj_bin.groupby(["condition_name", "trial_bin_index", "trial_bin_label"], dropna=False).agg(
        accuracy_mean=("accuracy", "mean"),
        accuracy_sem=("accuracy", sem_numeric),
        rt_mean_correct_mean=("rt_mean_correct", "mean"),
        rt_mean_correct_sem=("rt_mean_correct", sem_numeric),
        n_subject_bins=("subject_id", "size"),
    ).reset_index()
    cat = pd.Categorical(group_bin["condition_name"], categories=CONDITION_ORDER_NOMINAL, ordered=True)
    group_bin = group_bin.assign(_order=cat).sort_values(["_order", "trial_bin_index"]).drop(columns="_order")

    # ------------------------------------------------------------------
    # B) Rolling curves: rolling accuracy and RT per subject-condition
    # ------------------------------------------------------------------
    roll_rows = []
    for (subject_id, condition_name), sub in learning.groupby(["subject_id", "condition_name"], dropna=False):
        sub = sub.sort_values("trial_in_condition").copy()
        sub["rolling_accuracy"] = sub["is_correct"].rolling(
            window=int(rolling_window), min_periods=max(3, int(rolling_window)//3)
        ).mean()

        # correct-trial RT only; incorrect trials become NaN, then rolling mean
        rt_vals = sub["voice_onset_rt_sec"].where(sub["is_correct"] == 1)
        sub["rolling_rt_correct"] = rt_vals.rolling(
            window=int(rolling_window), min_periods=2
        ).mean()
        ...
        for _, r in sub.iterrows():
            roll_rows.append({
                ...
                "trial_in_condition": int(r["trial_in_condition"]),
                "rolling_accuracy": r["rolling_accuracy"],
                "rolling_rt_correct": r["rolling_rt_correct"],
            })

    subj_roll = pd.DataFrame(roll_rows)
    if not subj_roll.empty:
        group_roll = subj_roll.groupby(["condition_name", "trial_in_condition"], dropna=False).agg(
            rolling_accuracy_mean=("rolling_accuracy", "mean"),
            rolling_accuracy_sem=("rolling_accuracy", sem_numeric),
            rolling_rt_correct_mean=("rolling_rt_correct", "mean"),
            rolling_rt_correct_sem=("rolling_rt_correct", sem_numeric),
            n_subjects=("subject_id", "nunique"),
        ).reset_index()
        cat = pd.Categorical(group_roll["condition_name"], categories=CONDITION_ORDER_NOMINAL, ordered=True)
        group_roll = group_roll.assign(_order=cat).sort_values(["_order", "trial_in_condition"]).drop(columns="_order")
    else:
        group_roll = pd.DataFrame()
    ...
    for (subject_id, condition_name), sub in learning.groupby(["subject_id", "condition_name"], dropna=False):
        sub = sub.sort_values("trial_in_condition").copy()
        last_acc = sub["is_correct"].tail(min(27, len(sub)))
        acc_conv = float(pd.to_numeric(last_acc, errors="coerce").mean()) if len(last_acc) else np.nan

        subc = sub[(sub["is_correct"] == 1) & sub["voice_onset_rt_sec"].notna()].copy()
        subc = subc[(subc["voice_onset_rt_sec"] >= 0.05) & (subc["voice_onset_rt_sec"] <= 8.0)]
        last_rt = subc["voice_onset_rt_sec"].tail(min(10, len(subc)))
        rt_conv = float(last_rt.mean()) if len(last_rt) else np.nan

        rows.append({
            "subject_id": subject_id,
            "condition_name": condition_name,
            "n_learning_trials": len(sub),
            "accuracy_convergence": acc_conv,
            "rt_convergence": rt_conv,
            "last_trial_index": int(sub["trial_in_condition"].max()) if len(sub) else np.nan,
        })

    conv_subj = pd.DataFrame(rows)
    if conv_subj.empty:
        conv_group = pd.DataFrame()
    else:
        conv_group = conv_subj.groupby("condition_name", dropna=False).agg(
            n_subjects=("subject_id", "nunique"),
            n_subject_condition=("subject_id", "size"),
            n_learning_trials_mean=("n_learning_trials", "mean"),
            n_learning_trials_sem=("n_learning_trials", sem_numeric),
            accuracy_convergence_mean=("accuracy_convergence", "mean"),
            accuracy_convergence_sem=("accuracy_convergence", sem_numeric),
            rt_convergence_mean=("rt_convergence", "mean"),
            rt_convergence_sem=("rt_convergence", sem_numeric),
            last_trial_index_mean=("last_trial_index", "mean"),
        ).reset_index()
        conv_group["condition_type"] = conv_group["condition_name"].map(CONDITION_TYPE)
        cat = pd.Categorical(conv_group["condition_name"], categories=CONDITION_ORDER_NOMINAL, ordered=True)
        conv_group = conv_group.assign(_order=cat).sort_values("_order").drop(columns="_order")

    return subj_bin, group_bin, subj_roll, group_roll, conv_group
...
def plot_condition_curves(
    ...
    """Save condition curves using binned means and rolling means."""
    curve_dir.mkdir(parents=True, exist_ok=True)

    # -------------------------------
    # Accuracy: binned mean curve
    # -------------------------------
    if not group_bin.empty:
        fig, ax = plt.subplots(figsize=(10.5, 5.8))
        for cname in CONDITION_ORDER_NOMINAL:
            sub = group_bin[group_bin["condition_name"] == cname].sort_values("trial_bin_index")
            if sub.empty:
                continue
            ax.plot(sub["trial_bin_index"], sub["accuracy_mean"], marker="o", label=cname)
            if "accuracy_sem" in sub.columns:
                low = sub["accuracy_mean"] - sub["accuracy_sem"].fillna(0)
                high = sub["accuracy_mean"] + sub["accuracy_sem"].fillna(0)
                ax.fill_between(sub["trial_bin_index"], low, high, alpha=0.15)
            if not conv_group.empty and cname in set(conv_group["condition_name"]):
                row = conv_group.loc[conv_group["condition_name"] == cname].iloc[0]
                y = row.get("accuracy_convergence_mean", np.nan)
                if pd.notna(y):
                    ax.axhline(y, linestyle="--", linewidth=0.8, alpha=0.25)
                    x_end = float(sub["trial_bin_index"].max())
                    ax.text(x_end + 0.15, y, f"≈{y:.2f}", fontsize=8, va="center")
        ax.set_title("Accuracy change by condition (binned group mean)")
        ax.set_xlabel("Trial bin (9 trials/bin)")
        ax.set_ylabel("Accuracy")
        ax.set_ylim(0, 1.05)
        ax.grid(alpha=0.3)
        ax.legend(fontsize=8, ncol=2)
        fig.tight_layout()
        fig.savefig(curve_dir / "accuracy_curve_by_condition_binned.png", dpi=220)
        plt.close(fig)

        fig, axes = plt.subplots(2, 3, figsize=(12, 7), sharex=False, sharey=True)
        axes = axes.flatten()
        for ax, cname in zip(axes, CONDITION_ORDER_NOMINAL):
            sub = group_bin[group_bin["condition_name"] == cname].sort_values("trial_bin_index")
            ax.set_title(cname)
            if not sub.empty:
                ax.plot(sub["trial_bin_index"], sub["accuracy_mean"], marker="o")
                if "accuracy_sem" in sub.columns:
                    low = sub["accuracy_mean"] - sub["accuracy_sem"].fillna(0)
                    high = sub["accuracy_mean"] + sub["accuracy_sem"].fillna(0)
                    ax.fill_between(sub["trial_bin_index"], low, high, alpha=0.15)
                if not conv_group.empty and cname in set(conv_group["condition_name"]):
                    ...
                    y = row.get("accuracy_convergence_mean", np.nan)
                    if pd.notna(y):
                        ax.axhline(y, linestyle="--", linewidth=1.0, alpha=0.6)
                        ax.text(0.98, 0.05, f"수렴≈{y:.2f}", transform=ax.transAxes, ha="right", va="bottom", fontsize=8)
            ax.set_ylim(0, 1.05)
            ax.grid(alpha=0.3)
            ax.set_xlabel("Trial bin")
            ax.set_ylabel("Accuracy")
        fig.suptitle("Condition-wise accuracy curves (binned mean)", y=1.02)
        fig.tight_layout()
        fig.savefig(curve_dir / "accuracy_curve_by_condition_binned_panels.png", dpi=220, bbox_inches="tight")
        plt.close(fig)

        # RT: binned correct-trial mean curve
        fig, ax = plt.subplots(figsize=(10.5, 5.8))
        for cname in CONDITION_ORDER_NOMINAL:
            sub = group_bin[group_bin["condition_name"] == cname].sort_values("trial_bin_index")
            sub = sub[sub["rt_mean_correct_mean"].notna()]
            if sub.empty:
                continue
            ax.plot(sub["trial_bin_index"], sub["rt_mean_correct_mean"], marker="o", label=cname)
            if "rt_mean_correct_sem" in sub.columns:
                low = sub["rt_mean_correct_mean"] - sub["rt_mean_correct_sem"].fillna(0)
                high = sub["rt_mean_correct_mean"] + sub["rt_mean_correct_sem"].fillna(0)
                ax.fill_between(sub["trial_bin_index"], low, high, alpha=0.15)
            if not conv_group.empty and cname in set(conv_group["condition_name"]):
                row = conv_group.loc[conv_group["condition_name"] == cname].iloc[0]
                y = row.get("rt_convergence_mean", np.nan)
                if pd.notna(y):
                    ax.axhline(y, linestyle="--", linewidth=0.8, alpha=0.25)
                    x_end = float(sub["trial_bin_index"].max())
                    ax.text(x_end + 0.15, y, f"≈{y:.2f}s", fontsize=8, va="center")
        ax.set_title("Voice onset RT change by condition (binned group mean)")
        ax.set_xlabel("Trial bin (9 trials/bin)")
        ax.set_ylabel("Voice onset RT (s)")
        ax.grid(alpha=0.3)
        ax.legend(fontsize=8, ncol=2)
        fig.tight_layout()
        fig.savefig(curve_dir / "rt_curve_by_condition_binned.png", dpi=220)
        plt.close(fig)
        ...
        for ax, cname in zip(axes, CONDITION_ORDER_NOMINAL):
            ...
            ax.set_title(cname)
            if not sub.empty:
                ax.plot(sub["trial_bin_index"], sub["rt_mean_correct_mean"], marker="o")
                if "rt_mean_correct_sem" in sub.columns:
                    low = sub["rt_mean_correct_mean"] - sub["rt_mean_correct_sem"].fillna(0)
                    high = sub["rt_mean_correct_mean"] + sub["rt_mean_correct_sem"].fillna(0)
                    ax.fill_between(sub["trial_bin_index"], low, high, alpha=0.15)
                if not conv_group.empty and cname in set(conv_group["condition_name"]):
                    ...
                    if pd.notna(y):
                        ax.axhline(y, linestyle="--", linewidth=1.0, alpha=0.6)
                        ax.text(0.98, 0.05, f"수렴≈{y:.2f}s", transform=ax.transAxes, ha="right", va="bottom", fontsize=8)
            ax.grid(alpha=0.3)
            ax.set_xlabel("Trial bin")
            ax.set_ylabel("RT (s)")
        fig.suptitle("Condition-wise RT curves (binned mean)", y=1.02)
        fig.tight_layout()
        fig.savefig(curve_dir / "rt_curve_by_condition_binned_panels.png", dpi=220, bbox_inches="tight")
        plt.close(fig)

    # -------------------------------
    # Rolling curves
    # -------------------------------
    if not group_roll.empty:
        fig, ax = plt.subplots(figsize=(10.5, 5.8))
        for cname in CONDITION_ORDER_NOMINAL:
            sub = group_roll[group_roll["condition_name"] == cname].sort_values("trial_in_condition")
            if sub.empty:
                continue
            ax.plot(sub["trial_in_condition"], sub["rolling_accuracy_mean"], label=cname)
        ax.set_title("Accuracy change by condition (subject-level rolling mean)")
        ax.set_xlabel("Trial index within condition")
        ax.set_ylabel("Rolling accuracy")
        ax.set_ylim(0, 1.05)
        ax.grid(alpha=0.3)
        ax.legend(fontsize=8, ncol=2)
        fig.tight_layout()
        fig.savefig(curve_dir / "accuracy_curve_by_condition_rolling.png", dpi=220)
        plt.close(fig)

        fig, ax = plt.subplots(figsize=(10.5, 5.8))
        for cname in CONDITION_ORDER_NOMINAL:
            sub = group_roll[group_roll["condition_name"] == cname].sort_values("trial_in_condition")
            sub = sub[sub["rolling_rt_correct_mean"].notna()]
            if sub.empty:
                continue
            ax.plot(sub["trial_in_condition"], sub["rolling_rt_correct_mean"], label=cname)
        ax.set_title("Voice onset RT change by condition (subject-level rolling mean)")
        ax.set_xlabel("Trial index within condition")
        ax.set_ylabel("Rolling correct-trial RT (s)")
        ax.grid(alpha=0.3)
        ax.legend(fontsize=8, ncol=2)
        fig.tight_layout()
        fig.savefig(curve_dir / "rt_curve_by_condition_rolling.png", dpi=220)
        plt.close(fig)


def plot_convergence_bars(conv_group: pd.DataFrame, out_dir: Path):
    if conv_group.empty:
        return
    save_bar(
        conv_group,
        x="condition_name",
        y="accuracy_convergence_mean",
        yerr="accuracy_convergence_sem",
        title="Converged accuracy by condition",
        ylabel="Estimated converged accuracy",
        out=out_dir / "bar_converged_accuracy_by_condition.png",
    )
    save_bar(
        conv_group,
        x="condition_name",
        y="rt_convergence_mean",
        yerr="rt_convergence_sem",
        title="Converged RT by condition",
        ylabel="Estimated converged RT (s)",
        out=out_dir / "bar_converged_rt_by_condition.png",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze tactile consonant learning experiment logs. v9: organized outputs and binned/rolling condition curves.")
    ...
    parser.add_argument("--curve-bin-size", type=int, default=9, help="Trial bin size for condition curves. Default: 9 trials.")
    parser.add_argument("--curve-rolling-window", type=int, default=9, help="Rolling window size for smoothed condition curves. Default: 9 trials.")
    ...
    df.to_csv(dirs["tables"] / "all_trials_clean.csv", index=False, encoding="utf-8-sig")
    ...
    cond_bin_subj, cond_bin_group, cond_roll_subj, cond_roll_group, condition_convergence = condition_trial_curves(df, bin_size=args.curve_bin_size, rolling_window=args.curve_rolling_window)
    ...
    # Save CSV outputs.
    subj_summary.to_csv(dirs["condition"] / "subject_condition_summary.csv", index=False, encoding="utf-8-sig")
    group_summary.to_csv(dirs["condition"] / "group_condition_summary.csv", index=False, encoding="utf-8-sig")
    subj_block.to_csv(dirs["curves"] / "subject_learning_curve_by_miniblock.csv", index=False, encoding="utf-8-sig")
    group_block.to_csv(dirs["curves"] / "group_learning_curve_by_miniblock.csv", index=False, encoding="utf-8-sig")
    symbol_curve.to_csv(dirs["curves"] / "consonant_learning_curve_by_exposure.csv", index=False, encoding="utf-8-sig")
    position_curve.to_csv(dirs["curves"] / "position_learning_curve_by_exposure.csv", index=False, encoding="utf-8-sig")
    symbol_overall.to_csv(dirs["item_learning"] / "consonant_overall_learning_summary.csv", index=False, encoding="utf-8-sig")
    position_overall.to_csv(dirs["item_learning"] / "position_overall_learning_summary.csv", index=False, encoding="utf-8-sig")
    effect_summary.to_csv(dirs["effects"] / "symbol_position_effect_summary.csv", index=False, encoding="utf-8-sig")
    symbol_count_detail.to_csv(dirs["item_learning"] / "consonant_exposures_to_learn_subject_condition.csv", index=False, encoding="utf-8-sig")
    position_count_detail.to_csv(dirs["item_learning"] / "position_exposures_to_learn_subject_condition.csv", index=False, encoding="utf-8-sig")
    symbol_count_group.to_csv(dirs["item_learning"] / "consonant_exposures_to_learn_group.csv", index=False, encoding="utf-8-sig")
    position_count_group.to_csv(dirs["item_learning"] / "position_exposures_to_learn_group.csv", index=False, encoding="utf-8-sig")
    sym_rt_subj.to_csv(dirs["rt"] / "consonant_rt_subject_phase.csv", index=False, encoding="utf-8-sig")
    pos_rt_subj.to_csv(dirs["rt"] / "position_rt_subject_phase.csv", index=False, encoding="utf-8-sig")
    sym_rt_group.to_csv(dirs["rt"] / "consonant_rt_group_phase.csv", index=False, encoding="utf-8-sig")
    pos_rt_group.to_csv(dirs["rt"] / "position_rt_group_phase.csv", index=False, encoding="utf-8-sig")
    sym_rt_curve.to_csv(dirs["rt"] / "consonant_rt_curve_by_exposure.csv", index=False, encoding="utf-8-sig")
    pos_rt_curve.to_csv(dirs["rt"] / "position_rt_curve_by_exposure.csv", index=False, encoding="utf-8-sig")
    cond_bin_subj.to_csv(dirs["curves"] / "condition_accuracy_rt_curve_binned_subject.csv", index=False, encoding="utf-8-sig")
    cond_bin_group.to_csv(dirs["curves"] / "condition_accuracy_rt_curve_binned_group.csv", index=False, encoding="utf-8-sig")
    cond_roll_subj.to_csv(dirs["curves"] / "condition_accuracy_rt_curve_rolling_subject.csv", index=False, encoding="utf-8-sig")
    cond_roll_group.to_csv(dirs["curves"] / "condition_accuracy_rt_curve_rolling_group.csv", index=False, encoding="utf-8-sig")
    condition_convergence.to_csv(dirs["condition"] / "condition_convergence_summary.csv", index=False, encoding="utf-8-sig")
    ...
    if not symbol_count_group.empty and "exposures_to_learn_mean" in symbol_count_group.columns:
        symbol_count_group.sort_values("exposures_to_learn_mean", ascending=True).to_csv(
            dirs["item_learning"] / "rank_fastest_learned_consonants.csv", index=False, encoding="utf-8-sig"
        )
    if not position_count_group.empty and "exposures_to_learn_mean" in position_count_group.columns:
        position_count_group.sort_values("exposures_to_learn_mean", ascending=True).to_csv(
            dirs["item_learning"] / "rank_fastest_learned_positions.csv", index=False, encoding="utf-8-sig"
        )
    if not sym_rt_group.empty and "rt_mean" in sym_rt_group.columns:
        sym_rt_group[sym_rt_group["phase"] == "learning"].sort_values("rt_mean", ascending=True).to_csv(
            dirs["rt"] / "rank_fastest_rt_consonants_learning.csv", index=False, encoding="utf-8-sig"
        )
    if not pos_rt_group.empty and "rt_mean" in pos_rt_group.columns:
        pos_rt_group[pos_rt_group["phase"] == "learning"].sort_values("rt_mean", ascending=True).to_csv(
            dirs["rt"] / "rank_fastest_rt_positions_learning.csv", index=False, encoding="utf-8-sig"
        )

    # Save Excel workbook if possible.
    try:
        with pd.ExcelWriter(dirs["tables"] / "analysis_summary.xlsx", engine="openpyxl") as writer:
            ...
            cond_bin_group.to_excel(writer, sheet_name="cond_bin_group_trial", index=False)
            cond_roll_group.to_excel(writer, sheet_name="cond_roll_group_trial", index=False)
            condition_convergence.to_excel(writer, sheet_name="condition_convergence", index=False)
    except Exception as e:
    ...
    save_bar(
        ...
        out=dirs["condition"] / "bar_trials_to_criterion.png",
    )
    save_bar(
        group_summary,
        x="condition_name",
        ...
        out=dirs["condition"] / "bar_retention_accuracy.png",
    )
    save_bar(
        group_summary,
        x="condition_name",
        ...
        out=dirs["condition"] / "bar_criterion_reached_rate.png",
    )
    plot_learning_curve(group_block, dirs["curves"] / "learning_curve_by_condition_miniblock.png")
    plot_symbol_position_curves(symbol_curve, position_curve, dirs["curves"])
    plot_item_learning_count_bars(symbol_count_group, position_count_group, dirs["item_learning"])
    plot_rt_item_bars(sym_rt_group, pos_rt_group, dirs["rt"])
    plot_rt_exposure_curves(sym_rt_curve, pos_rt_curve, dirs["rt"])
    plot_condition_curves(cond_bin_group, cond_roll_group, condition_convergence, dirs["curves"])
    plot_convergence_bars(condition_convergence, dirs["condition"])
    plot_heatmaps(df, dirs["effects"])
    plot_confusion_matrices(df, dirs["confusion"])

    write_markdown_report(dirs["report"], df, subj_summary, group_summary, effect_msg)

    print("Analysis complete.")
    print(f"Input: {data_dir}")
    print(f"Output: {out_dir.resolve()}")
    print(output_index_message())
    print("Key files:")
    print(f"  - {dirs['tables'] / 'analysis_summary.xlsx'}")
    print(f"  - {dirs['report'] / 'analysis_report.md'}")
    print(f"  - {dirs['condition'] / 'group_condition_summary.csv'}")
