...
Usage
-----
python analyze_tactile_consonant_learning_v1.py --data-dir data/logs --out-dir analysis_results

If your CSV files are somewhere else:
python analyze_tactile_consonant_learning_v1.py --data-dir C:\path\to\logs --out-dir C:\path\to\analysis_results
"""
...
def learning_curves(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    subj_block = learning.groupby(["subject_id", "session_id", "condition_name", "mini_block_calc"], dropna=False).agg(
        ...
        rt_mean_correct=("voice_onset_rt_sec", lambda s: np.nanmean(s[learning.loc[s.index, "is_correct"] == 1]) if len(s) else np.nan),
...
def main():
    parser = argparse.ArgumentParser(description="Analyze tactile consonant learning experiment logs.")
    ...
    symbol_overall = df[df["phase"] == "learning"].groupby("correct_symbol").agg(
        ...
        n_trials=("is_correct", "size"),
        rt_mean_correct=("voice_onset_rt_sec", lambda s: np.nanmean(s[df.loc[s.index, "is_correct"] == 1]) if len(s) else np.nan),
    ).reset_index()
